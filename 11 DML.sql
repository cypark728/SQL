...
-- 50번 부서는 EMPLOYEE테이블에서 참조되고 있기 때문에 삭제가 일어나면 참조무결성 제약이 위배됩니다. 삭제가 안됨!
...
-- 2ND - MERGE문으로 직접 특정 데이터의 값을 넣고자 할 떄 사용할 수 있음
MERGE INTO EMPS E1
USING DUAL
...
-- 사본테이블 만들기 (연습용으로 씀)
CREATE TABLE EMP1 AS (SELECT * FROM EMPLOYEES); ---데이터 구조 + 데이터 복사
SELECT *FROM EMP1;
...
SELECT * FROM DEPTS;
UPDATE DEPTS
SET DEPARTMENT_NAME = 'IT bank' WHERE DEPARTMENT_NAME LIKE 'IT Support';

--2. department_id가 290인 데이터의 manager_id를 301로 변경
UPDATE DEPTS
SET MANAGER_ID = 301 WHERE DEPARTMENT_ID = 290;
...
WHERE DEPARTMENT_NAME LIKE 'IT Helpdesk';
...
--1. 부서명 영업을 삭제 하세요
SELECT * FROM DEPTS WHERE DEPARTMENT_NAME LIKE '영업';
DELETE FROM DEPTS WHERE DEPARTMENT_ID = 320;

--2. 부서명 NOC를 삭제하세요
SELECT DEPARTMENT_ID FROM DEPTS WHERE DEPARTMENT_NAME LIKE 'NOC';
DELETE FROM DEPTS WHERE DEPARTMENT_NAME LIKE 'NOC';
...
INSERT INTO DEPTS VALUES('IT_DEV', '아이태개발팀', 6000, 20000);
