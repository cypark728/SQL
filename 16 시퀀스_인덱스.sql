...
-- 생성
CREATE SEQUENCE DEPTS SEQ; -- 기본값으로 지정이 되면서 시퀀스가 생성됩니다.
...
-- 시퀀스 사용 방법 2가지
SELECT DEPTS_SEQ.CURRVAL FROM DUAL; --NEXTVAL가 한번 실행이 되고난 이후에 확인이 가능하빈다. 
...
-- 시퀀스 증가값을 (-1 * 현재값)으로 바꾸고
SELECT DEPTS_SEQ.CURRVAL FROM DUAL;
ALTER SEQUENCE DEPTS_SEQ INCREMENT BY -139;
-- 전진을 시킴
...
INSERT INTO EMPS VALUES(TO_CHAR(SYSDATE, 'YYYY') || '-' || LPAD(EMPS_SEQUENCE.NEXTVAL,5,0) , 'EXAMPLE');
SELECT * FROM EMPS;
...
-- 비고유인덱스는 일반 컬럼에 지정해서 조회를 빠르게 할 수 잇는 인덱스입니다.
-- 단, INDEX는 조회를 빠르게 하지만, 무자위하게 많이 사용되면 오히려 성능저하를 나타낼 수도 있습니다.
