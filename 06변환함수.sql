...
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') FROM DUAL; -- 데이트 포멧형시깅 아닌값은 ""로 묶어야함.
SELECT FIRST_NAME, TO_CHAR(HIRE_DATE, 'YY-DD-MM AM') FROM EMPLOYEES;

--TO_CHAR - 숫자 -> 문자로 강제 형변환 (숫자 포멧형식 쓰입니다)
SELECT TO_CHAR(20000, '99999999999') FROM DUAL; -- 9는 자리수
SELECT TO_CHAR(20000, '09999999999') FROM DUAL; -- 남는 자리는 0으로 채움
SELECT TO_CHAR(20000, '9999') FROM DUAL; -- 자리가 부족하면 # 으로 처리
SELECT TO_CHAR(20000.123, '99999.999') FROM DUAL;
SELECT TO_CHAR(20000, '$999,999.99') FROM DUAL;
SELECT TO_CHAR(20000, 'L999,999.99') FROM DUAL; -- $는 그냥 쓰면 됩니다. L을 붙이면 지역 화폐 기호
...
SELECT FIRST_NAME
    ...
    ,CASE JOB_ID WHEN 'IT_PROG' THEN SALARY * 1.1
                ...
                WHEN 'AD_VF' THEN SALARY  * 1.3
                ELSE SALARY
    END
FROM EMPLOYEES;

-- @ND - WHEN 조건식을 넣을 수도 있음
SELECT FIRST_NAME
    , JOB_ID
    , SALARY
    , CASE WHEN JOB_ID = 'IT_PROG' THEN SALARY * 1.1
            WHEN JOB_ID = 'FI_MGR' THEN SALARY * 1.2
            WHEN JOB_ID = 'AD_VF' THEN SALARY * 1.3
        END
FROM EMPLOYEES;

-- COALESCE(값, 값, ....) 코얼레스 - NULL이 아닌 첫번쨰 인자값을 반환하는 함수
...
SELECT FIRST_NAME
    ...
    ,TO_CHAR((SALARY * NVL(COMMISSION_PCT, 0) + SALARY * 1300), 'L99,999,999') AS 급여
    ,DECODE(MOD(TRUNC((SYSDATE - HIRE_DATE)/365), 5), 0, '진급대상', '진급대상아님') AS 진급상태
