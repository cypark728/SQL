...
-- 자주 사용되는 컬럼만 지정해 놓으면, 관리가 용이해집니다.
-- 뷰는 물리적으로 데이터가 저장된 형태는 아니고, 원본테이블을 기반으로한 논리적 테이블입니다. 
...
-- 뷰의 옵션
-- 뷰 생성문장에 마지막에 넣습니다.
-- WITH CHECK OPTION - WHERE 절에 들어간 컬럼의 변경이나, 추가를 금지 제약
-- WITH READ ONLY - 읽기전용뷰, DML구분 금지
CREATE OR REPLACE VIEW VIEW_EMP
AS(
    SELECT EMPLOYEE_ID,
           FIRST_NAME,
